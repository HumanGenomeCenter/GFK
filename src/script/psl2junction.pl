...
my $multiThres = 20; # if the read is mapped to more than this parameter value then skip this read
...
foreach my $key (sort keys %junc2count) {
    print OUT1 $key . "\t" . $junc2count{$key}->[0] . "\t" . $junc2count{$key}->[1] . "\n";
...
sub procRecords {
    ...
    if ($cons1 == 1) {
        ...
        if (not exists $junc2ID{$key}) {
            ...
            $junc2ID{$key} = $junc2ID{$key} . "," . $Qname[0];
        }
